...
logPath = "/home/huazhang/openwhisk_tmp_dir/wsklogs/controller0/controller0_logs.log"
...
def ConstructTestDataframe(since, limit=1000, read_results=False):
    ...
    for activation in activations:
        ...
        perf_data['activationId'].append(activation['_id'][6:])
...
def main(argv):
    ...
    for line in logFile.readlines():
        if line.find('[INFO]') == -1:
            continue
        idx = line.find(']')
        timeStamp = getTimeStamp(line[1:idx])
        if timeStamp < test_start_time:
            continue
        elif line.find('scheduled') == -1 :
            continue
        else:
            mp = re.search(pattern='invoker\d+/\d+', string=line)
            invoker = mp.group()
            try:
                cnt = invokerCnt[invoker]
                invokerCnt[invoker] = cnt + 1
            except Exception:
                invokerCnt[invoker] = 1
    logFile.close()
    print(invokerCnt)
    ...
    for dim in ['initTime', 'execution', 'latency']:
        ...
        print('Median ' + dim + ' (ms): ' + str(test_df[dim].median()))
        print('***********')
    ...
    for dim in ['initTime', 'execution', 'latency']:
        ...
        print('Median '  + dim + ' (ms): ' + str(warm_starts_test_df[dim].median()))
        print('***********')
    ...
    for dim in ['initTime', 'execution', 'latency']:
        ...
        print('Median ' + dim + ' (ms): ' + str(cold_starts_test_df[dim].median()))
        print('***********')
